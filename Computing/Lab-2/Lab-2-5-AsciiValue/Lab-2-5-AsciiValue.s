...
;
	...
	LDR R1, =0	;note: R1 must be defined and outside the loop.
doGet
	BL	get
	BL	put
...
ifnum	CMP R0, #0x30	; compare if input is
	BLT en		;between ascii charac. 0-9
	CMP R0, #0x39
	BGT en
	SUB R0,R0,#0x30 ;subtract 0x30 to get the decimal value counterpart
	MOV R2,R0	;move the input to R2
	
	LDR R4, =10	;to move the digits to the left multply by 10
	
	MUL R1,R4,R1	;store the value of 10 * the value in r0. initially, r1 is 0
	ADD R1,R2,R1	;add value stored in R2 and the r1 in r1. the loop repeats
	
	
	
	
en	CMP	R0, #0x0D	; Stop when RETURN is entered (ASCII 0x0A)
	BNE	doGet
